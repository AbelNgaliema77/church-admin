...
public sealed class WorkersController : ControllerBase
{
    ...
    public async Task<ActionResult<WorkerResponse>> Create(CreateWorkerRequest request)
    {
        ...
        bool emailExists = await _db.Workers.AnyAsync(x => x.Email == email);
        ...
        Worker createdWorker = await _db.Workers
            .Include(x => x.WorkerTeams)
            .ThenInclude(x => x.Team)
            .FirstAsync(x => x.Id == worker.Id);

        WorkerResponse response = WorkerResponse.FromEntity(createdWorker);
        ...
        UpdateWorkerRequest request)
    {
        ...
        await _auditService.LogAsync(
            ...
            AuditAction.Updated,
            before: before,
            after: WorkerResponse.FromEntity(worker));
    ...
    public async Task<IActionResult> AssignToTeam(
        Guid id,
        AssignWorkerTeamRequest request)
    {
        FluentValidation.Results.ValidationResult validation =
        ...
        bool alreadyCurrentMember = worker.WorkerTeams.Any(x =>
            x.TeamId == request.TeamId &&
            x.EndDate is null);

        if (alreadyCurrentMember)
        {
            return BadRequest("Worker already belongs to this team.");
        }
        ...
        await _auditService.LogAsync(
            ...
            after: request,
            reason: "Assigned worker to team");
    ...
    public async Task<IActionResult> RemoveFromTeam(Guid id, Guid teamId)
    {
        ...
        object before = new
        {
            workerTeam.WorkerId,
            workerTeam.TeamId,
            workerTeam.RoleInTeam,
            workerTeam.StartDate,
            workerTeam.EndDate
        };
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        worker.DeletedBy = User.Identity?.Name ?? "system";
